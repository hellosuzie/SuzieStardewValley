...
void MineState2::ChangeEnd()
{
	SCore::CurState = ALLSTATE::MINE;
}

MineState2::MineState2()
{
}
...
void MineState2::StateInit()
{
	Mine2Sp = GETSOUND;

	MineNum* Num = MakeActor<MineNum>();
	Num->NumRender()->SetSprite(L"MineNum.bmp", 2);
	Num->Pos({ 50, 40 });

	Ladder* m_Ladder = MakeActor<Ladder>();
	m_Ladder->Pos(Player::IndexWorldPos({ 6, 5 }));
	m_Ladder->m_Count = 2;

	Fade* F = MakeActor<Fade>();

	m_MineBack = MakeActor<MineBack>();
	m_MineTile = MakeActor<MineTile2>();
...
void MineState2::PlayerDirCheck(IPOS _Pos)
{
...
void MineState2::PickAxeWork(IPOS _Pos)
{
	PlayerDirCheck(_Pos);
	...
	if (nullptr != m_RFindIter->second)
	{
		...
		if (true == RockCheck)
		{
			Mine2Sp->NewSoundPlay(L"take_stone.wav");
