...
void MineState5::ChangeEnd()
{
	SCore::CurState = ALLSTATE::MINE;
	MineState::MineSp0->Stop();
}

MineState5::MineState5()
{
}
...
void MineState5::StateInit()
{
	Mine5Sp = GETSOUND;

	MineNum* Num = MakeActor<MineNum>();
	Num->NumRender()->SetSprite(L"MineNum.bmp", 5);
	Num->Pos({ 50, 40 });
	...
	m_MineTile = MakeActor<MineTile5>();
	...
	//m_MainMonster = MakeActor<Monster>(0);
	//m_MainMonster->Pos({ 500, 700 });
	//m_MainMonster->SetDirPlayer(MAINPLAYER);
	//m_MainMonster->MonsterAtt(5);

	//Monster* m_SecondMonster = MakeActor<Monster>(1);
	//m_SecondMonster->Pos({ 400, 800 });
	//m_SecondMonster->SetDirPlayer(MAINPLAYER);
...
void MineState5::PlayerDirCheck(IPOS _Pos)
{
...
void MineState5::PickAxeWork(IPOS _Pos)
{
	PlayerDirCheck(_Pos);
	...
	if (nullptr != m_RFindIter->second)
	{
		...
		if (true == RockCheck)
		{
			Mine5Sp->NewSoundPlay(L"take_stone.wav");
